// ============================================================
// CHEESEPLOT — Sovereign P2P Geodetic Surveyor & Cadastre
// app.js — Full Feature Implementation
// ============================================================

// ── Global State ─────────────────────────────────────────────
const state = {
  wallet:    { connected: false, address: null, ncheese: 0 },
...
function bindWallet() {
  document.getElementById('btn-connect-wallet').addEventListener('click', async () => {
    ...
    try {
      ...
      state.wallet.ncheese   = parseFloat((Math.random() * 120 + 5).toFixed(2));

      setText('wallet-address-display', `${state.wallet.address.slice(0,6)}…${state.wallet.address.slice(-4)}`);
      setText('ncheese-balance', `${state.wallet.ncheese} NCHEESE`);
      document.getElementById('ncheese-balance-wrap').classList.remove('hidden');
...
function bindNTRIP() {
  const presets = {
    ...
    'depin-manila': { url: 'depin.cheeseplot.io:2101',  mount: 'CHZ_RTCM3'   },
    'depin-cebu':   { url: 'cebu.cheeseplot.io:2101',   mount: 'CHZ_CEBU3'   },
    'depin-davao':  { url: 'davao.cheeseplot.io:2101',  mount: 'CHZ_DAVAO3'  },
  };
...
function startNTRIP() {
  ...
  state.intervals.ntrip = setInterval(() => {
    state.ntrip.packets++;
    state.wallet.ncheese = parseFloat((state.wallet.ncheese + 0.001).toFixed(4));
    setText('ncheese-balance', `${state.wallet.ncheese} NCHEESE`);
...
function exportGeoJSON() {
  ...
  triggerDownload(`cheeseplot-${Date.now()}.geojson`, JSON.stringify(gj, null, 2), 'application/geo+json');
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Document>
    <name>CheesePlot Lot Export</name>
    <Placemark>
  ...
  triggerDownload(`cheeseplot-${Date.now()}.kml`, kml, 'application/vnd.google-earth.kml+xml');
...
async function downloadCertificate() {
  ...
  doc.text('CHEESEPLOT — SOVEREIGN CADASTRAL CERTIFICATE', W / 2, 12, { align: 'center' });
  ...
  doc.text('This certificate is generated by the CheesePlot Sovereign Cadastral System (cheeseplot-cadastre.web.app).', W / 2, 282, { align: 'center' });
  ...
  doc.save(`CheesePlot-Certificate-${parcelId}.pdf`);
...
function saveMyLots()  { localStorage.setItem('cheeseplot_lots', JSON.stringify(state.myLots)); }
function loadMyLots()  { try { state.myLots = JSON.parse(localStorage.getItem('cheeseplot_lots') || '[]'); } catch { state.myLots = []; } }
